...
export default function ChatWidget() {
  const { isOpen, hasUnread, toggleChat, messages, addMessage } = useChatStore();
          ...
          <div className="flex-1 p-3 overflow-y-auto bg-gray-50 text-sm flex flex-col gap-3">
            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <span className={`inline-block p-2 rounded-lg ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-black'}`}>
                  {msg.content}
                </span>
